...
int [monster] __voting_setting_monster_priorities =
{
	...
	$monster[government bureaucrat]:2, //nice for absentee ballots, but
	$monster[Terrible mutant]:1, //technically optimal in-run, but the effect is minor
	$monster[Slime blob]:4, //harder to fight in-run
	$monster[Annoyed snake]:3,
};
...
void voteInVotingBooth(boolean allow_interacting_with_user)
{
	...
	initiative_priorities["State-mandated bed time of 8PM."] = 99; //+1 Adventure(s) per day
	...
	if (king_liberated)
		initiative_priorities["Repeal leash laws."] = 100; //+2 Familiar Experience Per Combat
